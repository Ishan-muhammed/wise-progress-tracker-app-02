...
BEGIN
  ...
  -- Handle classes if provided
  IF NEW.raw_user_meta_data ? 'classes' THEN
    classes_array := ARRAY(SELECT jsonb_array_elements_text(NEW.raw_user_meta_data->'classes'));
    
    -- Insert each class
    FOREACH class_name IN ARRAY classes_array LOOP
      INSERT INTO public.user_classes (user_id, class)
      VALUES (NEW.id, class_name)
